...
// import { signup } from "../mocks/auth"; // Commented out mock auth
...
export default function Signup() {
  ...
  const handleFormSubmission = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { data, error } = await signUp(emailAddress, passwordField, { name: fullName, role: userRole });

      if (error) {
        throw error;
      }

      // We'll insert values in the table during profile setup
      // The signup is just for authentication, and the user needs to verify
      // their email before profile completion

      // Show confirmation message about email verification
      navigate('/confirmation', {
        state: {
          message: "Please check your email to verify your account. Once verified, you'll be redirected to complete your profile.",
          email: emailAddress
        }
      });
    } catch (err: any) {
  ...
  <main className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
    <div className="w-full max-w-md p-8 bg-white rounded-2xl shadow-lg flex flex-col gap-6">
      ...
      <div className="flex justify-center gap-4 mb-2">
        <button
          type="button"
          className={`px-4 py-2 rounded-lg font-semibold transition border ${userRole === "student"
        ...
        <button
          type="button"
          className={`px-4 py-2 rounded-lg font-semibold transition border ${userRole === "admin"
              ? "bg-blue-600 text-white"
              : "bg-gray-100 text-blue-700"
            }`}
      ...
      <form className="flex flex-col gap-4" onSubmit={handleFormSubmission}>
        ...
        <div>
          <input
            ...
            className={`px-4 py-2 rounded-lg border focus:outline-none focus:ring-2 focus:ring-blue-200 w-full ${emailValidationError ? 'border-red-500' : 'border-gray-200'
              }`}
            value={emailAddress}
